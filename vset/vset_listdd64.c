...
mdd_create_stack()
{
    ...
    Print( "initial stack size %"PRIu64, stack_size);
}
...
mdd_push(uint64_t mdd)
{
    if (mdd_top==stack_size) {
        ...
        Debug( "new stack size %"PRIu64"", stack_size);
    }
...
static void mdd_collect(uint64_t a,uint64_t b){
    ...
    Debug( "ListDD64 garbage collection: %"PRIu64" of %"PRIu64" nodes used",
            mdd_used, mdd_nodes);
    ...
    if (mdd_used > fib(nodes_fib-1) && mdd_nodes != UINT64_MAX) {
        ...
        Debug( "new op cache has %"PRIu64" entries", new_cache_size);
    }
    ...
    if (!resize){
        ...
        Debug("copied %"PRIu64" op cache nodes",copy_count);
        RTfree(op_cache);
        op_cache=new_cache;
        ...
        Debug("node/unique tables have %"PRIu64"/%"PRIu64" entries",mdd_nodes,uniq_size);
    }
}
...
vdom_t vdom_create_list64_native(int n){
    Print("Creating a native ListDD 64-bit domain.");
    ...
    if (unique_table==NULL) {
        ...
        Print("initial node table has %"PRIu64" entries",mdd_nodes);
        uniq_size=(nodes_fib + 1 <= FIB_MAX)?fib(nodes_fib+1):UINT64_MAX;
        Print("initial uniq table has %"PRIu64" entries",uniq_size);
        cache_size=(nodes_fib+cache_fib <= FIB_MAX)?fib(nodes_fib+cache_fib):UINT64_MAX;
        Print("initial op cache has %"PRIu64" entries",cache_size);
