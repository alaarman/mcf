...
mdd_create_stack()
{
    ...
    mdd_stack = malloc(sizeof(uint32_t[stack_size]));
    Print( "initial stack size %" PRIu32, stack_size);
}
...
mdd_push(uint32_t mdd)
{
    if (mdd_top==stack_size) {
        ...
        Debug( "new stack size %" PRIu32, stack_size);
    }
...
static void mdd_collect(uint32_t a,uint32_t b){
    ...
    */
    Debug( "ListDD garbage collection: %" PRIu32" of %" PRIu32" nodes used",
            mdd_used, mdd_nodes);
    int resize=0;
    ...
    if (mdd_used > fib(nodes_fib-1) && mdd_nodes != UINT32_MAX) {
        Debug("insufficient free nodes, resizing");
        resize=1;
        nodes_fib++;
        ...
        if (new_cache_size < cache_size) {
            Debug( "op cache reached maximum size");
            new_cache_size = UINT32_MAX;
        }
        Debug( "new op cache has %" PRIu32 " entries", new_cache_size);
    }
    ...
    if (!resize){
        ...
        Debug("copied %" PRIu32" op cache nodes",copy_count);
        RTfree(op_cache);
        op_cache=new_cache;
        ...
        Debug("node/unique tables have %u/%u entries",mdd_nodes,uniq_size);
    }
}
...
static uint32_t mdd_create_node(uint32_t val,uint32_t down,uint32_t right){
    ...
    //Debug( "mdd_create_node: slot_hash=%u, slot=%u, res=%u", slot_hash, slot, res);
    while(res){
...
//    mdd_load_node_ids = malloc(n_count*sizeof(uint32_t));
...
//        //Debug( "id=%llu, val=%u, down=%llu [%llu], right=%llu [%llu]",
...
set_create_mdd(vdom_t dom, int k, int *proj)
{
    ...
    int l = (k < 0)?0:k;
    vset_t set = (vset_t)malloc(sizeof(struct vector_set) + sizeof(int[l]));
    set->dom  = dom;
    set->mdd  = 0;
...
set_copy_match_mdd(vset_t dst, vset_t src, int p_len, int *proj, int *match)
{
    ...
    //Print( "set_copy_match_mdd: p_id=%d", p_id);
...
set_copy_match_proj_mdd(vset_t dst, vset_t src, int p_len, int *proj, int p_id, int *match)
{
    ...
    //Print( "set_copy_match_proj_mdd: p_id=%d", p_id);
    // The p_id of is shifted in hash keys; check this is ok
...
proj_create_mdd(int p_len, int *proj)
{
    assert(p_len >= 0);
    uint32_t p_id = mdd_put(0, (uint32_t*)proj, p_len, NULL); // FIXME: enable to pass a projection to this function!
    //Print( "proj_create_mdd: p_id=%d", p_id);
    // The p_id of is shifted in hash keys; check this is ok
    if ((p_id >> 16) > 0) Abort("set_copy_match_mdd: projection identifier too large");
    return p_id;
}
...
set_least_fixpoint_mdd(vset_t dst, vset_t src, vrel_t rels[], int rel_count)
{
    ...
    top_groups = malloc(sizeof(top_groups_info[init_state_len]));
    proj_set = malloc(sizeof(vset_t[rel_count]));

    for (int lvl = 0; lvl < init_state_len; lvl++) {
        top_groups[lvl].top_groups = malloc(sizeof(int[rel_count]));
...
vdom_t vdom_create_list_native(int n){
    Print("Creating a native ListDD domain.");
    ...
    if (unique_table==NULL) {
        ...
        Print("initial node table has %" PRIu32 " entries",mdd_nodes);
        uniq_size=(nodes_fib + 1 <= FIB_MAX)?fib(nodes_fib+1):UINT32_MAX;
        Print("initial uniq table has %" PRIu32 " entries",uniq_size);
        cache_size=(nodes_fib+cache_fib <= FIB_MAX)?fib(nodes_fib+cache_fib):UINT32_MAX;
        Print("initial op cache has %" PRIu32" entries",cache_size);

        unique_table=malloc(uniq_size*sizeof(uint32_t));
        node_table=malloc(mdd_nodes*sizeof(struct mdd_node));
