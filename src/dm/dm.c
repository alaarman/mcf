...
dm_print_perm(const matrix_header_t* p)
{
    for (int i = 0; i < p->size; i++) {
        ...
        Print("i %d, becomes %d,  at %d, group %d\n", i, u.becomes, u.at, u.group);
    }
}
...
dm_tc(matrix_t* tgt)
{
    assert (tgt->rows == tgt->cols && "dm_tc() not a square matrix.");
    for (int k = 0; k < tgt->rows; k++) {
    for (int i = 0; i < tgt->rows; i++) { // Pick all vertices as source one by one
    for (int j = 0; j < tgt->rows; j++) { // Pick all vertices as destination for the above picked source
        if (dm_is_set(tgt, i, k) && dm_is_set(tgt, k, j)) {
            dm_set (tgt, i, j);
        }
    }}}
}
...
merge_rows_(matrix_t* m, int rowa, int rowb)
{
    ...
    // make sure rowb > rowa
    if (rowa == rowb) exit(LTSMIN_EXIT_FAILURE);
...
merge_cols_(matrix_t* m, int cola, int colb)
{
    ...
    // make sure colb > cola
    if (cola == colb) exit(LTSMIN_EXIT_FAILURE);
...
print_progress(const matrix_t* const m, dm_cost_t cost, long double num_perms, rt_timer_t timer)
{
    ...
    case DM_EVENT_SPAN: {
        ...
        Print("Current costs: %.*g (%.*g)", sig_digs, cost, sig_digs, cost / (dm_ncols(m) * (double) dm_nrows(m)));
    } break;
    case DM_WEIGHTED_EVENT_SPAN: {
        int sig_digs = ceil(log10(dm_ncols(m)));
        const double cost = dm_weighted_event_span(m, NULL);
        Print("Current costs: %.*g", sig_digs, cost);
    } break;
    default:
        Exit(LTSMIN_EXIT_FAILURE, "unsupported cost function");
    }

    Trace("Anneal time: %.1f seconds, total permutations: %Lg, %.1Lf permutations/second",
...
dm_anneal(matrix_t* m, dm_cost_t cost, const int timeout)
{
    ...
    Print("Press ctrl+c to show current costs.");
    Print("Press ctrl+z to stop and use current permutation.");
...
dm_optimize(matrix_t* m)
{
    ...
    while (last_min != min) {
        ...
        if (best_i != best_j) {
            ...
            Trace ("best rotation: %d-%d, costs %d\n", best_i, best_j, min);
            if (DEBUG) dm_print (stdout, m);

            best_i = best_j = 0;
        }
    }
    Debug ("cost: %f ", dm_event_span(m, NULL));
}
...
dm_all_perm(matrix_t* m)
{
    ...
    while (1) {
        ...
        // debug
        if (DEBUG) {current_all_perm_ (perm, len);};
        Debug ("costs: %d\n", last_min);
...
{
    // check size
    Assert(bitvector_size(bv) == (size_t) dm_ncols(m));
...
{
    Assert(dm_nrows(a) == dm_nrows(b) && dm_ncols(a) == dm_ncols(b));
...
{
    Assert(dm_nrows(a) == dm_nrows(b) && dm_ncols(a) == dm_ncols(b));
    Assert(dm_equal_header(&a->row_perm, &b->row_perm) && dm_equal_header(&a->col_perm, &b->col_perm),
            "fast equals only possible on matrices with the same permutation");
...
{
    Assert(bitvector_size(tgt) == (size_t) dm_ncols(m) && bitvector_size(bv) == (size_t) dm_nrows(m));
...
dm_FORCE(matrix_t* m)
{
    ...
    Trace("FORCE took %d iterations", dm_ncols(m) - iterations);
}
