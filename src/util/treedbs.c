...
static void resize_hash(treedbs_t dbs,int node){
	...
	//Print(info,"rehash of %d to %d succesful",node,dbs->db_hash_size[node]);
}
...
TreeFold_ret(treedbs_t dbs,int *vector, int *idx)
{
    ...
    if (nPars==1) {
        ...
        if(vector[0]>=dbs->range){
            ...
            //Print(info,"resize from %d to %d",old,dbs->range);
        ...
        //Print(info,"insert %d as %d",vector[0],dbs->map[vector[0]]);
...
static void TreeInfoPrint(int depth,int node,treedbs_t dbs){
	...
	prefix[2*depth]=0;
	Print("%s%d: node has %d elements and %d hash slots",prefix,node,dbs->db_next[node],dbs->db_hash_size[node]);
...
void TreeInfo(treedbs_t dbs){
	if (dbs->nPars==1){
		Print("map with %d entries and range of %d elements.",dbs->count,dbs->range);
	} else {
