...
{
    Print("vset_sylvan: starting garbage collection");
}

VOID_TASK_0(gc_end)
{
    Print("vset_sylvan: garbage collection done");
}
...
ltsmin_initialize_sylvan()
{
    ...
    Print("Sylvan allocates %s virtual memory for nodes table and operation cache.", buf);
    to_h((1ULL<<tablesize)*24+(1ULL<<cachesize)*36, buf);
    Print("Initial nodes table and operation cache requires %s.", buf);
