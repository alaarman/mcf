                           ...
                           uint64_t exp, uint64_t new_val)
{
    ...
    Assert (new_val < (1UL << dbs->root.sat_bits), "new_val too high");
    Assert ((new_val & mask) == new_val, "new_val too high w.r.t. bits");
        ...
        uint64_t data, int index, uint64_t *res, loc_t *loc, bool insert)
{
    ...
    uint64_t            mem, hash, a;
    mem = hash = mix64 (data);
    Assert (data != EMPTY_1, "Value (%zu) out of table range.\n"
    ...
    for (size_t probes = 0; probes < nodes->thres; probes++) {
        ...
        Assert ((hash & CL_MASK) != (a & CL_MASK), "loop in hashing function");
        stat->rehashes++;
    }
                    ...
                    int satellite_bits, int slim, int indexing)
{
    ...
    Assert (nNodes >= 2, "Tree vectors too small: %d", nNodes);
    Assert ((slim == 0 || slim == 1), "Wrong value for slim: %d", slim);
