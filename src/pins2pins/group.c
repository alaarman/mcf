...
void
tree_ordering(rw_info_t *inf, const matrix_t *m, int *col_perm)
{
    //list_t **group2write = (list_t **) dm_rows_to_idx_table (inf->mayw);
    list_t **write2group = (list_t **) dm_cols_to_idx_table (inf->mayw);

    int k = dm_ncols (m);
    for (int i = 0; i < k; i++) {  // loop over slots

        list_t *groups = write2group[i]; // groups (actions) writing to slot i
        for (int *g = list_begin(groups); g != list_end(groups); g++) {
            if (SETTINGS.DBG) printf("Slot %d is written by group %d", i, *g);
        }

        col_perm[i] = (i + k/4) % k;  //  reorder
    }
}
...
apply_regroup_spec (rw_info_t *inf, const char *spec_, guard_t **guards, const char* sep)
{
    ...
    if (spec_ != NULL) {
        ...
        while ((tok = strsep (&spec, sep)) != NULL) {
            ...
            if (strcmp (tok, "sw") == 0) {
                Print ("Regroup Select Write matrix");
                selection = inf->mayw;
            } else if (strcmp (tok, "sr") == 0) {
                Print ("Regroup Select Read matrix");
                selection = inf->r;
            } else if (strcmp (tok, "sc") == 0) {
                Print ("Regroup Select Combined matrix");
                ...
                Print ("Regroup Select Event Span");
                cost = DM_EVENT_SPAN;
            } else if (strcasecmp (tok, "swes") == 0) {
                Print ("Regroup Select Weighted Event Span");
                cost = DM_WEIGHTED_EVENT_SPAN;
            } else if (strcasecmp (tok, "w2W") == 0) {
                Print ("Regroup over-approximate must-write to may-write");
                dm_clear(inf->mustw);
                dm_clear(inf->old_mustw);
            } else if (strcasecmp (tok, "r2+") == 0) {
                Print ("Regroup over-approximate read to read + write");
                dm_apply_or(inf->mayw, inf->r);
                dm_apply_or(inf->old_mayw, inf->old_r);
            } else if (strcmp (tok, "W2+") == 0) {
                Print ("Regroup over-approximate may-write \\ must-write to read + write");
                matrix_t w;
                dm_copy(inf->mayw, &w);
                ...
                Print ("Regroup over-approximate must-write to read + write");
                dm_apply_or(inf->r, inf->mustw);
                dm_apply_or(inf->old_r, inf->old_mustw);
            } else if (strcmp (tok, "-2r") == 0) {
                Print ("Regroup over-approximate copy to read");
                dm_fill(inf->r);
                dm_fill(inf->old_r);
            } else if (strcasecmp (tok, "rb4w") == 0) {
                Print ("Regroup Read BeFore Write");
                ...
                Print ("Regroup macro Simulated Annealing: %s", macro);
                apply_regroup_spec (inf, macro, guards, sep);
            } else if (strcasecmp (tok, "gs") == 0) {
                const char         *macro = "gc,gr,cw,rs";
                Print ("Regroup macro Group Safely: %s", macro);
                apply_regroup_spec (inf, macro, guards, sep);
            } else if (strcasecmp (tok, "ga") == 0) {
                const char         *macro = "ru,gc,rs,cw,rs";
                Print ("Regroup macro Group Aggressively: %s", macro);
                apply_regroup_spec (inf, macro, guards, sep);
            } else if (strcasecmp (tok, "gc") == 0) {
                const char         *macro = "cs,cn";
                Print ("Regroup macro Cols: %s", macro);
                apply_regroup_spec (inf, macro, guards, sep);
            } else if (strcasecmp (tok, "gr") == 0) {
                const char         *macro = "rs,rn";
                Print ("Regroup macro Rows: %s", macro);
                apply_regroup_spec (inf, macro, guards, sep);
            } else if (strcasecmp (tok, "tree") == 0) {
                Print ("Regroup cols for tree compressions");
                int col_perm[dm_ncols(selection)];
                tree_ordering(inf, selection, col_perm);
                apply_permutation(inf, NULL, col_perm);
            } else if (tok[0] != '\0') {
                Abort ("Unknown regrouping specification: '%s'", tok);
            }
            if (group_time) {
                RTstopTimer(trans_timer);
                RTprintTimer(trans_timer, "Transformation took");
...
str2vec(const int max_size, const char *perm, int *vec)
{
    ...
    while ((e = strsep(&p, ",")) != NULL && strlen(e) > 0) {
        if (i == max_size) {
            Abort ("to many numbers (%d) given", i + 1);
        }
        const int j = atoi(e);

        if (j < 0) {
            Abort ("%d at %d should be >= 0", j, i);
        }

        if (j >= max_size) {
            Abort ("%d at %d should be < %d", j, i, max_size);
        }

        for (int k = 0; k < i; k++) {
            if (j == vec[k]) {
                Abort ("%d at %d already given previously at %d", j, i, k);
            }
        }
    ...
    if (i != max_size) {
        Abort ("too few numbers");
    }
}

model_t
pins2pins_regroup (model_t model)
{
    if (!pins_has_guards(model)) {
        if (PINS_USE_GUARDS)
            Print ("Ignoring option " USE_GUARDS_OPTION " for lack of guards!");
        PINS_USE_GUARDS = 0;
    }

    if (SETTINGS.OPTIONS.REGROUP == NULL && col_ins == NULL &&
             row_perm == NULL && col_perm == NULL) {
        return model;
    }

    Print ("Initializing regrouping layer");
    ...
    if (row_perm != NULL) {
        Print ("Permuting rows according to given vector");
    ...
    if (col_perm != NULL) {
        Print ("Permuting columns according to given vector");
        int perm[dm_ncols(inf.r)];
        str2vec(dm_ncols(inf.r), col_perm, perm);
        apply_permutation(&inf, NULL, perm);
    }

    if (SETTINGS.OPTIONS.REGROUP != NULL) {
        Print ("Regroup specification: %s", SETTINGS.OPTIONS.REGROUP);
        if (PINS_USE_GUARDS) {
            apply_regroup_spec (&inf, SETTINGS.OPTIONS.REGROUP, GBgetGuardsInfo(model), ",");
        } else {
            apply_regroup_spec (&inf, SETTINGS.OPTIONS.REGROUP, NULL, ",");
        }
    }
    ...
    // fill statemapping: assumption this is a bijection
    {
        ...
        if (Nparts != lts_type_get_state_length (GBgetLTStype (model)))
            Abort ("state mapping in file doesn't match the specification");
        ctx->len = Nparts;
    ...
    // fill transition mapping: assumption: this is a surjection
    {
        ...
        Print  ("Regrouping: %d->%d groups", oldNgroups,
                 newNgroups);
    ...
    if (SETTINGS.DBG){
        lts_type_printf(ltstype);
    }
    Debug("permuting ltstype");
    ...
    RTstopTimer(t);
    RTprintTimer(t, "Regrouping took");
    RTdeleteTimer(t);
