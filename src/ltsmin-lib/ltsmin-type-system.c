        ...
        ltsmin_expr_t e, ltsmin_parse_env_t env, data_format_t l, data_format_t r)
{
    ...
    if (entry.error == -1) {
        ...
        Verbose("Error: expecting format pair (LHS, RHS) to be any pair in {%s}", hint);

        const char* ex = LTSminPrintExpr(e, env);
        Error("Incompatible type formats (LHS = \"%s\", RHS = \"%s\") for expression: \"%s\"." ,
        ...
        ltsmin_expr_t e, ltsmin_parse_env_t env, data_format_t c)
{
    ...
    if (entry.error == -1) {
        ...
        Verbose("Error: expecting format to be any of {%s}", hint);

        const char* ex = LTSminPrintExpr(e, env);
        Error("Incompatible type format (\"%s\") for expression: \"%s\"",
        ...
        if (other != NULL) {
            if (typeno == -1) {
                if (other->token != SVAR && other->token != EVAR) {
                    LTSminLogExpr("A chunk should be paired with a state variable,"
                ...
                if (other->token == SVAR || other->token == EVAR) {
                    ...
                    if (t != typeno) {
                        LTSminLogExpr("LHS and RHS are chunks and "
            ...
            LTSminLogExpr("A chunk should be paired.", e->parent, env);
    ...
    switch (e->node_type) {
        case BINARY_OP: {
            ...
            switch (e->token) {
                ...
                default: {
                    LTSminLogExpr("Unsupported expression: ", e, env);
                    Exit(LTSMIN_EXIT_FAILURE, " ");
                }
            }
        }
        ...
        case VAR: {
            ...
            Error("Invalid identifier: '%s' is not a state variable "
