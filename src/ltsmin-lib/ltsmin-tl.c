...
read_formula (const char *file)
{
    ...
    if (in) {
        ...
        Print("\"%s\" is not a file, parsing as formula...", file);
...
fill_env (ltsmin_parse_env_t env, lts_type_t ltstype)
{
    ...
    for (int i = 0; i < N; i++) {
        ...
        Assert (i == idx, "Model has equally named state variables ('%s') at index %d and %d", name, i, idx);
    }
    ...
    for (int i = 0; i < L; i++) {
        ...
        int idx = LTSminStateVarIndex(env,name);
        Assert (N + i == idx, "Model has equally named state labels ('%s') at index %d and %d", name, i - N, idx);
    }
...
{
    Assert(SIgetCount(env->idents) == 0); // TODO: why is this necessary? >1 formula?
    ...
    switch(in->token) {
        case MU_VAR: {
            for (int i=this->top-1; this->stack[i] != in->idx; i--) {
                ...
                Debug("Dependency %d -> %d", in->idx, this->stack[i]);
...
void tableaux_print_compressed(syntax_tree_t *s, int level, int* pstart, int* pstop)
{
    ...
    if (s->branch[ST_LEFT] != NULL) {
        ...
        //Print("after left/right start %d between %d stop %d", start, between, stop);
    }

    // at this point stop - start should be >= node_len
    // furthermore, start is fixed after this

    //Print("after node start %d stop %d", start, stop);
            ...
            case TABLEAUX_PRETERMINAL:
                {
                    ...
                    Verbose("encountered preterminal %s with companion %s", buf, buf1);
                    ...
                    if (s->node->quantifier == NODE_ALL) {
                        ...
                        Verbose(" nu-path exist from %s to %s? %s", buf, buf, path_var_idx ? "yes" : "no");
                    } else {
                        path_var_idx = find_path(MU_PATH, s, s->companion);
                        Verbose(" mu-path exist from %s to %s? %s", buf, buf, path_var_idx ? "yes" : "no");
                    }
...
ltsmin_expr_t ctl_star_to_mu(ltsmin_expr_t in)
{
    ...
    // debug print this
    if (VERBOSE) tableaux_print(t);

    // step 3: translate the tableaux
    ltsmin_expr_t mu = tableaux_translate_syntax_tree(t, &t->root);
    if (VERBOSE) ltsmin_expr_print_mu(mu, buf);
    Verbose("** %s **", buf);

    tableaux_destroy(t);
    Verbose("-- end conversion --");
