...
DVEexit()
{
    ...
    if (snprintf(rmcmd, sizeof rmcmd, "rm -rf %s", templatename) < (ssize_t)sizeof rmcmd) {
        ...
        if (status == -1 || !WIFEXITED(status) || WEXITSTATUS(status) != 0)
            Print("Could not remove %s.", templatename);
    }
}
...
DVE2compileGreyboxModel(model_t model, const char *filename)
{
    ...
    ret = system(command);
    Assert(ret == 0, "Command failed with exit code %d: %s\n"
...
void
DVE2loadModel(model_t model, const char *filename)
{
    lts_type_t ltstype;
    ...
    if (NULL == dlHandle) {
        ...
        Assert (extension != NULL, "No filename extension %s", filename);
        ++extension;
    ...
    if (!get_dna_matrix) {
        Print ("*** Warning ***");
        Print ("You are using an old version of our patched DiVinE compiler.");
        Print ("This might influence the performance of partial order reduction negatively.");
        Print ("Please download the latest from: http://fmt.cs.utwente.nl/tools/ltsmin/");
        Print ("*** Warning ***");
    } else {
