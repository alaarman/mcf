...
//        GBregisterLoader("pnml", PNMLloadModel);
//        GBregisterLoader("andl", ANDLloadModel);
//
...
{
    (void) model;
    Print ("max token count: %u", max_token_count);
}
    ...
    for (arc_t *arc = pn_context->arcs + pn_context->transitions[t].start; arc->transition == t; arc++) {
        switch(arc->type) {
            case ARC_IN: {
                ...
                Assert(out[arc->place] - arc->num >= 0, "transition should not have been disabled");
    ...
    for (arc_t *arc = pn_context->arcs + pn_context->transitions[t].start; arc->transition == t; arc++) {
        switch(arc->type) {
            case ARC_IN: {
                ...
                /* If there is an underflow then this transition is disabled,
                 * while it should not be, since this is the update function. */
                Assert(*place - arc->num >= 0, "transition should not have been disabled");
...
get_label_long(model_t model, int label, int *src) {
    pn_context_t *pn_context = GBgetContext(model);
    Assert(label < pn_context->num_guards, "unknown state label");
        ...
        if (node->type == XML_ELEMENT_NODE) {
            if (xmlStrcmp(node->name, (const xmlChar*) "place") == 0) {
                ...
                *toolspecific = node;
                ...
                Print("WARNING: ignoring \"finalmarkings\"");
                continue;
            }
        }
...
noack1(pn_context_t *pn_context)
{
    ...
    RTstopTimer(timer);
    if (DEBUG) RTprintTimer (timer, "Computing Noack1 order took");
    RTdeleteTimer(timer);
...
noack2(pn_context_t *pn_context)
{
    ...
    RTstopTimer (timer);
    if (DEBUG) RTprintTimer (timer, "Computing Noack2 order took");
    RTdeleteTimer (timer);
...
initGreyboxModel(model_t model)
{
    ...
    Trace("Determining Petri net size");
}
...
create_lts_type(model_t model)
{
    ...
    Print("Petri net has %d places, %d transitions and %d arcs",
        NUM_PLACES, NUM_TRANSS, pn_context->num_arcs);

    lts_type_t ltstype;

    Trace("Creating LTS type");
    ...
    Trace("Analyzing Petri net behavior");
}
...
set_dependencies(model_t model)
{
    ...
    Print("Petri net %s analyzed", pn_context->name);
    ...
    Print("There are %ssafe places", pn_context->has_safe_places ? "" : "no ");
...
//    }
    ...
    } else Trace("Since this net has 1-safe places, short next-state functions are not used");
    ...
    //if (PINS_POR) {
        Trace("Creating Do Not Accord matrix");
        ...
        Trace("Creating Guard Necessary Enabling Set matrix");
        ...
        Trace("Creating Guard Necessary Disabling Set matrix");
        ...
        Trace("Creating Do Not Left Accord (DNB) matrix");
    ...
    //} else Print ("Not creating POR matrices");

    RTstopTimer (pn_context->timer);
    if (DEBUG) RTprintTimer (pn_context->timer, "Loading Petri net took");
    RTdeleteTimer (pn_context->timer);
}

void
ANDLloadModel(model_t model, const char *name)
{
    FILE *f = fopen(name, "r");
    yyscan_t scanner;
...
void
PNMLloadModel(model_t model, const char *name)
{
    Print("Edge label is %s", edge_label_option);
