...
void
ETFloadModel(model_t model, const char *name)
{
    ...
    for(int i=0; i < dm_nrows(p_dm_info); i++) {
        ...
        Print("table %d has %d states and %d transitions",
    ...
    for(int i=0;i<type_count;i++){
        Print("Setting values for type %d (%s)",i,lts_type_get_type(ltstype,i));
