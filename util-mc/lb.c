...
lb_local_init (lb_t *lb, int id, void *arg)
{
    ...
    Assert (alloc_distance == CACHE_LINE_SIZE, "Wrong alignment in allocation");
    loc->idle = 0;
    loc->requests = 0;
    loc->received = 0;
    loc->max_load = 0;
         ...
         lb_split_problem_f split)
{
    ...
    for (size_t idx = 0; idx < j; idx++) {
        ...
        if (j - idx <= (lb->threads >> 1)) {
            Assert (get_idle (lb, oid), "Thread reactiveated before handoff complete");
...
lb_internal (lb_t *lb, int id, size_t my_load, lb_split_problem_f split)
{
    ...
    do {
        ...
        if ( idle ) {
            ...
            if ( all_idle ) {
                set_all_done (lb);
                Assert (my_load == 0, "Premature termination detection");
                break; // load == 0
            }
...
{
    Assert (threads <= lb_MAX_THREADS, "Only %zu threads allowed", lb_MAX_THREADS);
...
#endif
    size_t W = lb->threads;
    Assert ((val < (1ULL<<SHIFT)), "Overflow in reduce");
