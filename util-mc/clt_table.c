...
{
	do {
        Assert (pos > 0, "Cleary table overflow"); //TODO: return negative error (see dbs-ll.c)
		pos--;
...
clt_find_right_from (const clt_dbs_t* dbs, size_t pos)
{
	do {
		pos++;
		Assert (pos < dbs->size + dbs->b_space + dbs->b_space, "Cleary table overflow");
	} while (dbs->table[pos].occupied);
	return pos;
}
...
{
    Assert (k < (1UL << dbs->keysize), "Cleary table: invalid key");
    ...
    if (dir > 0) {
        ...
        if (dbs->table[idx].virgin) { // Only search for the element if the virgin bit is set
            ...
            while (c2 < 1 && dbs->table[j].occupied) {
                Assert (j < dbs->size + dbs->b_space + dbs->b_space, "Cleary table overflow");
        ...
        q = t_right;
        while (1) {
            Assert (q <  dbs->size + dbs->b_space + dbs->b_space, "Cleary table overflow");
...
{
    Assert (sizeof(clt_bucket_t) == sizeof(uint32_t), "Wrong bucket size");
    Assert (log_size <= 40, "Cleary table to large: 2^%u", log_size);
    Assert (log_size > 7, "Cleary table to small to add breathing space: 2^%u", log_size);
