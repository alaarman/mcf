...
int ETFgetType(etf_model_t model,const char*sort)
{
    ...
    Debug( "type %s %s (%d)", sort, is_new?"created":"present",type_no);
...
void etf_ode_add(etf_model_t model){
	...
	for(int i=0;i<state_length;i++){
		...
		Debug("var %d is %s with %d values",i,lts_type_get_state_name(model->ltstype,i),vcount[i]);
	}
	int is_new;
	...
	Assert (format == LTStypeEnum || format == LTStypeChunk);
	Debug("sign type has %d values",SIgetCount(model->type_values[signtype]));
	for(int i=0;i<SIgetCount(model->type_values[signtype]);i++){
		Debug("sign[%d]=%s",i,SIget(model->type_values[signtype],i));
	}
	...
	Debug("signs (-,0,+) are %d %d %d",neg,zero,pos);
	...
	for(int i=0;i<state_length;i++){
		...
		if(varidx>=state_length){
			...
			Debug("variable %s has index %d",var,varidx);
		}
		...
		if (ETFrelCount(trans)){
			Debug("adding trans section with %d entries",ETFrelCount(trans));
			ensure_access(model->trans_manager,model->trans_count);
			model->trans[model->trans_count]=trans;
			model->trans_count++;
		} else {
			Debug("skipping empty trans section");
		}
	}
}
