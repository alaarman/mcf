...
#include <util/util.h>
    ...
    void yyerror(YYLTYPE *loc, void *scanner, andl_context_t *andl_context, const char* c) {
        Print("Parse error on line %d: %s", loc->first_line, c);
...
pdec
    ...
    |   IDENT error SEMICOLON {
            Print("Something went wrong with place %s on line %d", $1, @1.first_line);
        }
    ;
...
tdec
    ...
    |   IDENT error SEMICOLON {
            Print("Something went wrong with transition %s on line %d", $1, @1.first_line);
        }
    ;
...
arc
    :   LBRAC IDENT op const_function RBRAC {
            ...
            if (i == SI_INDEX_FAILED) {
                andl_context->error = 1;
                Print("Place %s is not declared", $2);
            } else {
                ensure_access(andl_context->arc_man, andl_context->pn_context->num_arcs);
    ...
    |   LBRAC error RBRAC {
            Print("Missing identifier on line %d", @1.first_line);
        }
    |   LBRAC IDENT error RBRAC {
            Print("Something went wrong with arc %s on line %d", $2, @1.first_line);
        }
    ;
...
op
    ...
    |   ASSIGN {
            andl_context->error = 1;
            Print("Constant assign arcs not yet supported.");
        }
    ;
