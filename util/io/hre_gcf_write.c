...
#include "util/io/arch_object.h"
#include "util/io/gcf_common.h"
#include "util/io/stream_object.h"
#include "util/io/user.h"
...
archive_t arch_gcf_create(raf_t raf,size_t block_size,size_t cluster_size,int worker,int worker_count){
    archive_t arch=(archive_t)RTmallocZero(sizeof(struct archive_s));
    arch_init(arch);
    arch->raf=raf;
    ...
    arch->buffer=RTmallocZero(cluster_size);
