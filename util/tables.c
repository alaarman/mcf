...
void MTclusterBuild(matrix_table_t mt,int col,uint32_t count){
    ...
    Debug("replace column value with position in array");
    for(uint32_t i=0;i<mt->count;i++){
        mt->column[col][i]=--mt->begin[mt->column[col][i]];
    }
    Debug("moving rows to correct positions");
    ...
    for(uint32_t i=0;i<mt->count;i++){
        ...
        MTgetRow(mt,i,row1);
        //Debug("moving row %d to %d",i,pos1);
        for(;;){
            if (pos1==i) {
            ...
            //Debug("moving row %d to %d",pos1,pos2);
            ...
            //Debug("moving row %d to %d",pos2,pos1);
            MTgetRow(mt,pos2,row1);
            MTsetRow(mt,pos2,row2);
        }
    }
    Debug("deleting old column");
...
void MTsimplify(matrix_table_t dst, matrix_table_t src){
    ...
    Print("length of simplified list is %u",len);
    RTfree(next);
}
