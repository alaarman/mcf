...
chunk2string (chunk src, size_t dst_size, char *dst) {
    Debug("encoding chunk of length %d", src.len);

    Assert (dst_size >= sizeof("##..."), "Chunk too small (sizeof '##...')");
 ...
 /* quotable: */
    ...
    if (k+2 < dst_size) {
        ...
        Print("chunk overflow: truncating to %zu characters", dst_size-1);
        k = dst_size - sizeof "\"...";
...
#undef PUT_CHAR
	...
	Debug("return length is %d",dst->len);
}
