...
{
    Debug("Creating map of size %d.", size);
    map_t map;
    map.values = malloc(size*sizeof(entry_t));
...
bool simplemap_put(map_t map, uint32_t key, uint32_t value)
{
    ...
    assert(loc < map.size);
    //Debug("simplemap_put: key=%d, value=%d, loc=%d.", key, value, loc);
    uint32_t l = loc;
    uint32_t c = 0;
    while ((map.values[loc].value & 0x80000000) && map.values[loc].key != key)
    {
        ...
        if (loc==l)
        {
            Debug("simplemap_put: loc=%d, c=%d.", loc, c);
            Abort("Map is full.");
        }
        c++;
        assert(loc < map.size);
...
{
    Debug("Creating map of size %"PRIu64, size);
    map64_t map;
    map.values = malloc(size*sizeof(entry64_t));
    for(size_t i = 0; i < size; i++)
    {
        map.values[i].key = 0;
        map.values[i].value = 0;
...
bool simplemap64_put(map64_t map, uint64_t key, uint64_t value)
{
    ...
    while ((map.values[loc].value & 0x80000000) && map.values[loc].key != key)
    {
        ...
        if (loc==l)
        {
            Debug("simplemap_put: loc=%"PRIu64", c=%"PRIu64".", loc, c);
            Abort("Map is full.");
        }
        c++;
        assert(loc < map.size);
